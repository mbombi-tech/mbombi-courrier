...
/*if ($role === 'AP') {
    ...
    AND c.id IN (
        ...
        AND utilisateur_destination_id = ?
    )";
    $params[] = $user_id;
}*/
...
/* =======================
   TRI
======================= */
$sql .= " ORDER BY COALESCE(date_reception, c.date_creation) DESC";
...
<?php if ($role === 'Coordon'): ?>
<a href="ajouter_courrier.php" class="btn btn-success mb-3">➕ Nouveau courrier</a>
<?php endif; ?>
...
$badgeClass = 'bg-dark';
switch ($row['statut']) {
    case 'reçu': case 'recu': $badgeClass = 'bg-success'; break;
    case 'en traitement': $badgeClass = 'bg-warning text-dark'; break;
    case 'transféré': $badgeClass = 'bg-info text-dark'; break;
    case 'clôturé': $badgeClass = 'bg-secondary'; break;
}
?>
<tr>
<td><?= $i + 1 ?></td>
...
<td><span class="badge <?= $badgeClass ?>"><?= ucfirst($row['statut'] ?? '') ?></span></td>
<td><?= htmlspecialchars($row['service_source'] ?? '—') ?></td>
<td><?= date('d/m/Y H:i', strtotime($row['date_reception'] ?? $row['date_creation'] ?? 'now')) ?></td>
<td>
<a class="btn btn-sm btn-info" href="suivre_courrier.php?tracking_code=<?= urlencode($row['tracking_code'] ?? '') ?>">Suivre</a>

<?php if ($role === 'Coordon' && ($row['statut'] ?? '') !== 'clôturé'): ?>
<a class="btn btn-sm btn-warning" href="suivre_courrier6.php?tracking_code=<?= urlencode($row['tracking_code'] ?? '') ?>">
Soumettre</a>
...
<?php if ($role === 'AP'): ?>
<a class="btn btn-sm btn-warning" href="Transfert.php?tracking_code=<?= urlencode($row['tracking_code'] ?? '') ?>">Transférer</a>
<?php endif; ?>

<a class="btn btn-sm btn-secondary" href="historique.php?code=<?= urlencode($row['tracking_code'] ?? '') ?>">Historique</a>
</td>
</tr>
<?php endforeach; ?>
</tbody>
...
<?php else: ?>
<p class="text-center text-muted">Aucun courrier disponible.</p>
<?php endif; ?>
...
function fetchNotifications() {
    ...
    .then(data => {
        if (data.length > 0) {
            data.forEach(n => { alert(n.message); });
