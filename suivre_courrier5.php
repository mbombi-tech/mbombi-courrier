...
if (!empty($_GET['tracking_code'])) {
    ...
    if ($courrier) {
        ...
        ========================== */
		$stmt = $db->prepare("
        SELECT * FROM documents_courrier WHERE courrier_id = ?
         ");
        $stmt->execute([$courrier['id']]);
    ...
    <div class="card-body">
        ...
        <div class="row text-center">
            ...
            <div class="col-md-3">
                ...
                <strong><?= $courrier['tracking_code'] ?></strong>
            </div>

            <div class="col-md-3">
                <h6 class="text-muted">📂 Service actuel</h6>
                <strong><?= strtoupper($courrier['service_actuel']) ?></strong>
            </div>

            <div class="col-md-3">
                <h6 class="text-muted">⚙️ Statut</h6>
            ...
            <div class="col-md-3">
                <h6 class="text-muted">🕒 Dernière action</h6>
                <strong>
                    <?= !empty($historique)
...
<div class="card mb-4">
    <div class="card-header">
        👥 Équipe en charge du traitement
    </div>
...
// Statut exact venant de la base
$statut = trim($courrier['statut'] ?? '');

// Vérifications robustes (insensible à la casse)
$est_assigne     = strcasecmp($statut, 'assigné') === 0;
$est_a_orienter  = strcasecmp($statut, 'A orienter') === 0;
?>

<?php if (
    $est_assigne &&
    (in_array($role_id, $roles_autorises) || in_array($user_id, $est_assigné))
): ?>
        ...
        <?php if ($assignations): ?>
            <?php foreach ($assignations as $a): ?>
                <div class="border p-2 mb-2 d-flex justify-content-between align-items-center">
                    ...
                    <?php else: ?>
                        <span class="badge bg-warning">⏳ En attente ou en cours de traitement</span>
                    <?php endif; ?>
                </div>
                ...
                <?php if ($a['utilisateur_id'] == $user_id && $a['etat'] !== 'termine'): ?>
                    <a href="validation_courrier.php?courrier_id=<?= $courrier['id'] ?>&tracking_code=<?= urlencode($courrier['tracking_code']) ?>"
                       ...
                       onclick="return confirm('Terminer votre traitement ?')">
...
<!-- ================= ORIENTATION ================= -->
<div class="card mb-3">
    <div class="card-header">📤 Orientation du courrier</div>
    <div class="card-body">
        ...
        <?php if ($assignations): ?>
            <?php foreach ($assignations as $a): ?>
                <div class="border p-2 mb-2 d-flex justify-content-between align-items-center">
                    ...
                    <?php if ($a['etat'] === 'termine'): ?>
                        <span class="badge bg-success"> Orienté</span>
                    <?php else: ?>
                        <span class="badge bg-warning">⏳ En attente d’orientation</span>
                    <?php endif; ?>
                </div>

                <?php if ($a['utilisateur_id'] == $user_id && $a['etat'] !== 'termine'): ?>
                    <a href="orienter_courrier2.php?courrier_id=<?= $courrier['id'] ?>&tracking_code=<?= urlencode($courrier['tracking_code']) ?>"
                       class="btn btn-primary btn-sm mb-3"
                       onclick="return confirm('Orienter le courrier ?')">
                       📤 Orienter le courrier
                    </a>
                <?php endif; ?>

            <?php endforeach; ?>
        <?php else: ?>
            <p class="text-muted">Aucune orientation pour ce courrier.</p>
        <?php endif; ?>
...
<?php endif; ?>
    ...
    <div class="mb-5">
	  ...
	  <?php if (in_array($_SESSION['user']['role'], ['AP','agent','Directeur','SG','Coordon'])): ?> 
	  ...
	  <?php if (in_array($_SESSION['user']['role'], ['Directeur','AP','SG','Agent_courrier'])): ?>
    ...
    <a href="orienter_courrier.php?code=<?= urlencode($courrier['tracking_code']) ?>"
       class="btn btn-primary">📝 Pour orienter</a>
    <?php endif; ?>	
...
<?php if (
    ...
    strcasecmp($_SESSION['user']['role'], 'AP') === 0
): ?>

<a href="Valider_courrier.php?courrier_id=<?= intval($courrier['id']) ?>
&tracking_code=<?= urlencode($courrier['tracking_code']) ?>"
   class="btn btn-danger"
   onclick="return confirm('⚠️ Voulez-vous vraiment valider définitivement ce courrier ? Cette action est irréversible.')">
