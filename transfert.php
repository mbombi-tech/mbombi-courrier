...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($service_destination == $courrier['service_actuel_id']) {
        ...
        try {
            ...
            // 🔹 Nom du service destination
            $stmt = $db->prepare("SELECT nom FROM services WHERE id = ?");
            $stmt->execute([$service_destination]);
            $service_dest = $stmt->fetchColumn();

            // 🔹 LOGIQUE DU STATUT
            $nouveau_statut = $courrier['statut'];

            if (strtolower($service_dest) === 'secretariat general' 
                || strtolower($service_dest) === 'secrétariat général') {
