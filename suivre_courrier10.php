...
if (!empty($_GET['tracking_code'])) {
    ...
    if ($courrier) {
        ...
        /* Direction */
        if (in_array($role_name, ['AP','Directeur','SG'])) {
            $autorise = true;
        }


        /* Créateur */
        if ($courrier['cree_par'] == $user_id) {
            $autorise = true;
        }
        ...
        if (!$autorise) {
            ...
            $courrier = null;
            $erreur = "⛔ Vous n’êtes pas autorisé à consulter ce courrier.";
...
$nbAssign = count($assignations);
$estSoumis = !empty($courrier['date_soumission']);
$statut = strtolower(trim($courrier['statut']));
?>

<!-- ASSIGNER -->
<?php if (in_array($role_name, ['Directeur','AP','SG','Coordon'])): ?>
...
<!-- AJOUT DOCUMENT -->
<?php if (in_array($role_name, ['AP','agent','Directeur','SG'])): ?>
...
<!-- TRANSFERT -->
<?php if (in_array($role_name, ['AP','Directeur','SG'])): ?>
<a href="transfert.php?code=<?= urlencode($courrier['tracking_code']) ?>"
   class="btn btn-warning mb-2"
   onclick="return confirm('Voulez-vous vraiment transférer ?')">
   🔁 Transférer
</a>
<?php endif; ?>
...
<!-- SOUMETTRE -->
<?php if ($role_name === 'Coordon'): ?>
<a class="btn btn-warning mb-2"
   href="soumettre_courrier.php?tracking_code=<?= urlencode($courrier['tracking_code']) ?>">
   📤 Soumettre
</a>
<?php endif; ?>
...
<a href="cloturer_courrier.php?id=<?= intval($courrier['id']) ?>"
   ...
   onclick="return confirm('Voulez-vous vraiment clôturer ?')">
   🔒 Clôturer
</a>
<?php endif; ?>
...
<form method="POST"
      action="supprimer_courrier.php"
      onsubmit="return confirmerSuppression('<?= htmlspecialchars($courrier['tracking_code']) ?>');"
      class="d-inline">
...
function confirmerSuppression(code) {
    return confirm(
        "⚠️ Voulez-vous vraiment supprimer le courrier " + code + " ?\n\nCette action est définitive."
    );
}
</script>
